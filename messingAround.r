...
#b_date = paste(b_data[,1],"-",b_data[,2],"-",b_data[,3]," ",b_data[,4],":",00,sep="") ":",00,sep="") 
#b_time <- b_data[,2] + b_data[,3] + b_data[,4] + b_data[,5] 
b_date <- ymd_hm(b_date,tz = "UTC")
b_date <- b_date + minutes(4)
head(b_date)

b_data %>% select(V9) -> data
#data$time <- b_date

# date1 <- ymd_hms("2020-01-01 00:26:00")
# date2 <- ymd_hms("2020-01-01 00:56:00")

b_tbl <- tibble(
  ind = seq(as.interval(b_date,ymd_hms("2020-01-01 00:30:00"))),
  time = b_date,
  value = data
)
x <- seq(as.interval(b_date,ymd_hms("2020-01-01 00:30:00")))
buoy_ts <- as_tsibble(b_tbl,index=time,regular = TRUE)

# all_of(data) %>% 
#   mutate(time = ceiling_date(time, unit = "30 mins")) %>%
#   as_tsibble(data,regular=TRUE) -> buoy_ts
...
# buoy_ts %>% tsibble::update_tsibble(.data, regular = TRUE)


#fit <- buoy_ts %>% 
#model(m1 = ARIMA(value ~ pdq(1,1,0)) + PDQ(2,0,2))

# 
fit <- buoy_ts %>% model(auto = ARIMA(V9,approximation=FALSE))
#                          m1 = ARIMA(log(V9) ~ 0 + pdq(1,1,0) +PDQ(2,0,2))+season("2") )
report(fit)
glance(fit)

fit %>% forecast(h=30) %>% autoplot(buoy_ts)
report(fit)

# buoy_ts %>% fit %>% gg_tsdisplay(V9,"partial")
# 
# fit %>% autoplot()
